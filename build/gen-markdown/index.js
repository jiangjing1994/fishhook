...
const extractProps = (node) => {
  ...
  node.value.properties.forEach((prop) => {
    ...
    if (leadingComments) {
      if ('CommentLine' === leadingComments[0].type) {
        props[name].desc = leadingComments[0].value.trim()
      } else if ('CommentBlock' === leadingComments[0].type) {
        // 提取props说明
        let desc = leadingComments[0].value.match(/\*\s*[^@]\s*(.*)/)
        if (desc) {
          props[name].desc = desc[1]
        }
        // 提取方法返回值
        let res = leadingComments[0].value.match(/(@returns)[\s]*(.*)/)
        if (res) {
